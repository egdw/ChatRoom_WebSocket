...
public class WebServiceHandler extends TextWebSocketHandler {
    ...
    protected void handleTextMessage(WebSocketSession session, TextMessage message) throws Exception {
        ...
        System.out.println("获取到的客户端返回的数据:" + new String(bytes));
        String s = Constant.getSessionByClass(session);
        System.out.println(s);
        Constant.sendMessage(Constant.getSessionByClass(session), new String(bytes));
    }
    ...
    public void handleTransportError(WebSocketSession session, Throwable exception) throws Exception {
        ...
        Message message = new Message(400, "消息传输错误", "尊贵的服务器", Constant.getSessionByClass(session), SimpleDateFormat.getInstance().format(new Date()));
...
//        System.out.println("afterConnectionClosed");
        Constant.removeSession(session);
    }
...
//        System.out.println("getPrincipal:" + session.getPrincipal());
        ...
        Constant.addSession(random, session);
